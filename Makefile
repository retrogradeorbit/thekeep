...
APP=build/js/compiled/thekeep.js
IDX=build/index.html
IMG=build/img/tiles2.png build/img/font.png
...
REPO=git@github.com:retrogradeorbit/thekeep.git
...
itch.io: all
	rm -rf release thekeep.zip
	cp -a build release
	rm -rf release/.git
	cd release && zip -r ../thekeep.zip .
